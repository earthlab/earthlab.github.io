...
## Create fire boundary extent box
...
crop_all_tifs <- function(the_file, crop_extent, compress=F, aStack=F){
  
  # create raster or stack
  if(aStack==F){
    the_file_raster <- raster(the_file)
  } else{
    the_file_raster <- stack(the_file)
  }
...
# get crop extent
fire_crop_box <- readOGR("data/week6/vector_layers/fire_crop_box_2000m.shp")
...
# crop second landsat data
all_tifs <- data.frame(theFiles=list.files("data/week6/landsat/LC80340322016189-SC20170128091153",
                                           pattern=".tif$",
                                           full.names=T))
...
# crop second landsat data
all_tifs_173 <- data.frame(theFiles=list.files("data/week6/landsat/LC80340322016173-SC20170227185411",
                                           pattern=".tif$",
                                           full.names=T))

for(a_tif_file in all_tifs_173$theFiles){
  crop_all_tifs(a_tif_file,
                fire_crop_box) # our crop extent object
}
...
```{r}
# open naip
naip <- raster("data/week6/ortho/naip/m_3910505_nw_13_1_20150919/m_3910505_nw_13_1_20150919.tif")

plot(naip,
     col=gray(0:100 / 100))

# open naip
naip_stack <- stack("data/week6/ortho/naip/m_3910505_nw_13_1_20150919/m_3910505_nw_13_1_20150919.tif")
plotRGB(naip_stack)
# open fire boundary
fire_boundary <- readOGR("data/week6/vector_layers/fire_crop_box_500m.shp")
plot(fire_boundary, add=T)
...
# get crop extent
fire_crop_box_500 <- readOGR("data/week6/vector_layers/fire_crop_box_500m.shp")
# crop naip 2013 data
naip_2013 <- "data/week6/naip/m_3910505_nw_13_1_20130926/m_3910505_nw_13_1_20130926.tif"
naip_2015 <- "data/week6/naip/m_3910505_nw_13_1_20150919/m_3910505_nw_13_1_20150919.tif"

# crop data
crop_all_tifs(naip_2013,
                fire_crop_box_500, compress=T, aStack=T) # our crop extent object

crop_all_tifs(naip_2015,
                fire_crop_box_500, compress=T, aStack=T) # our crop extent object



# import data 
test <- stack("data/week6/naip/m_3910505_nw_13_1_20130926/crop/m_3910505_nw_13_1_20130926_crop.tif")
test

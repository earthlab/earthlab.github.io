...
fire_boundary_utm <- readOGR("data/week6/vector_layers/fire-boundary-geomac/co_cold_springs_20160711_2200_utm13n.shp")
```
...
# crop data
landsat_nbr_diff <- crop(landsat_nbr_diff, fire_boundary_utm)
...
plot(fire_boundary_utm, add=T, lwd=2)
...
freq(landsat_nbr_diff_class, useNA='no')
...
plot(landsat_nbr_diff_class,
     ...
     main="Landsat difference NBR - Post Fire \n Julian Day 205")
plot(fire_boundary_utm, add=T, lwd=2)
legend(landsat_nbr_diff_class@extent@xmax, landsat_nbr_diff_class@extent@ymax,
       c("Enhanced Regrowth", "Unburned", "Low Severity", "Moderate Severity", "High Severity"),
       fill=nbr_colors,
       cex=.9, bty="n", xpd=T)
plot(landsat_nbr_pre, zlim=c(-1,1), 
     box=F, axes=F,
     main="pre-fire NBR", ext=extent(fire_boundary_utm))
plot(fire_boundary_utm, add=T, lwd=2)
plot(landsat_nbr_post, zlim=c(-1,1),
     box=F, axes=F,
     main="post-fire NBR", ext=extent(fire_boundary_utm))
plot(fire_boundary_utm, add=T, lwd=2)
```

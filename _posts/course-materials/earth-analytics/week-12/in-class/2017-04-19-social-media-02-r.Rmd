...
Let's take a look at the twitter APIs
...
<figcaption>Graphic that shows the distrubtion of tweets across the Denver / Boulder region src: http://www.socialmatt.com/amazing-denver-twitter-visualization/</figcaption>
</figure>

To begin, we will need to setup an application in twitter that we can use to access
tweets. 

To do so - follow the documentation from rtweet here:

<a href="https://cran.r-project.org/web/packages/rtweet/vignettes/auth.html" target="_blank">setup twitter app</a>

http://www.socialmatt.com/amazing-denver-twitter-visualization/



## twitter in R  

We will use the rtweet package to 
...
```{r}
# load twitter library - this library is recommended now over twitteR
library(rtweet)
...
appname <- "earth-analytics-course"

## api key (example below is not a real key)
key <- "YbHGDrZ02nbj6bFvJSv8R3t9V"

## api secret (example below is not a real key)
secret <- "3ZtgoXhGslB8ip3yugmrfYBD3UwWcY4Bc4Kfqvk0QyBDJRquzd"
```
...
Finally, we can create a token that authenticates our access to tweets!
Note that the authentication process below will open a window in your browser.

If it works, it should render the following message in a browser window:
...
tweets that use the #rstats hashtag.
...
# remove retweets
team_rstats <- search_tweets("#rstats", n = 1500,
                             include_rts = FALSE)
team_rstats

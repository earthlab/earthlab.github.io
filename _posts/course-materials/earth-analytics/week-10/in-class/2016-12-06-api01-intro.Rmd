...
computer and into R, reading in data stored in text format from a website, into a data.frame in R and finally, accessing subsets of particular data using REST API calls in R. "
...
This week we will discuss programmatic access of data via direct downloads of data
and the use of Applied Programming Interfaces (APIs). Up until this point, we
...
Then we proceed to work with it in `R`. The data that we have downloaded
...
We can automate the data download process -
particularly when we are trying to download lots of data or particular subsets
of data to support an analysis if and when there are programmatic ways to
access the data online. When we automate data access, download / retrieval and
embed it in our code, we are linking our analysis to our data in a more direct
and better documented way. By doing this our code not only reminds our future
self of our process - it also reminds our future self where we got the data
...
1. **Tabular Human-readable file:** Files that are tabular, include csv's (Comma Separated Values) and even spreadsheets (MS Excel, etc). These files are organized into columns and rows and "flat" in structure rather than hierarchical.
...
Notice that the data that we downloaded above using `download.file()` is
**tabular** and thus **human-readable**. It contains a tabular
with rows and columns that we can quickly understand. `R` can import these data
into a `data.frame` structure and we can work with it programmatically.

However, would happens if our data structure is more complex? For example,
...
<!-- Maybe clarify whether the stuff below is the data (response) or a query -->
```json

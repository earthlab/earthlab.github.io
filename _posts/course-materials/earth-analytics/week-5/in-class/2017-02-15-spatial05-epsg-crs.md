...
projected and geographic coordinate reference systems. In this lessons we will
...
We can break down the proj 4 string into it's individual components (again, separated by + signs) as follows:
...
if there is no S, then you can assume it's north.
...
<i class="fa fa-star"></i> **Data Tip2:** sometimes you will encounter global data layers where the longitude spanes from 0-360 rather than -180 - 180. You can use the `rotate()` function to transform a raster into -180-180 to deal with this.
{: .notice}
...
EPSG acronym. Each code is a four-five digit number which represents a particular
CRS definition.
...
`rgdal`package in R.  can see a list of 
...
Let's search our datframe for 4326 to figure out what CRS it represents.
We can use dplyr pipes and the `filter()` function to quickly select this. 
...
If we use the stringr package, we can find all CRS' definitions that contain longlat.
...
How about ones that contain UTM
...
We can create CRS objects using both the text string itself and / or the epsg code. Let's give it a try. we will use our geographic definition as an example
...
Similarly we can use an epsg code to create a proj 4 string.
...
We won't spend a lot of time on the WKT format however, it's important to recoganize given some tools - like ESRI's ArcMap use this format as a standard.  Well-known Text (WKT) allows for compact machine- and human-readable representation of
geometric objects as well as to concisely describing the critical elements of
...
`
Sometimes WKT structured CRS information are embedded in a meatadata file - similar to the structure seen below:  
...
While we won't spent a lot of time on this format in this class, it's important
to atleast be able to recognaize it.


## How to look up a CRS 

The most powerful website to explore CRS information...
